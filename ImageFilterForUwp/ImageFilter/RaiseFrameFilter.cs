...
{
    public class RaiseFrameFilter : IImageFilter
    {
        int _size;
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if ((x < _size) && (y < height - x) && (y >= x))
                        cr = MyImage.rgb(255, 255, 65); // left
                    else if ((y < _size) && (x < width - y) && (x >= y))
                        cr = MyImage.rgb(255, 255, 120); // top
                    else if ((x > width - _size) && (y >= width - x) && (y < height + x - width))
                        cr = MyImage.rgb(0, 0, 65); // right
                    else if (y > height - _size)
                        cr = MyImage.rgb(0, 0, 120); // bottom
                    else
                        continue;

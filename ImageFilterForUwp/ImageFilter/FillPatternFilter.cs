...
namespace HaoRan.ImageFilter
{
    ...
    {
        private MyImage pattern;

        private float Mixture = 0.2f;//0.5f

        public FillPatternFilter(string path)
        {
            this.pattern = MyImage.LoadImage(path);
        }

        public FillPatternFilter(string path, int Mixture)
        {
            this.pattern = MyImage.LoadImage(path);
        ...
        public MyImage process(MyImage imageIn)
        {
            ...
            for (int x = 0; x < imageIn.getWidth(); x++)
            {
                for (int y = 0; y < imageIn.getHeight(); y++)
                {
                    ...
                    r1 = MyImage.SAFECOLOR(r + pattern.getRComponent(xx, yy));
                    g1 = MyImage.SAFECOLOR(g + pattern.getGComponent(xx, yy));
                    b1 = MyImage.SAFECOLOR(b + pattern.getBComponent(xx, yy));
                    r = (r * mix2) + (r1 * mix1);
                    g = (g * mix2) + (g1 * mix1);
                    b = (b * mix2) + (b1 * mix1);

...
{
    public class BilinearDistort : IImageFilter
    {
        protected MyImage clone;
	
	    public BilinearDistort(){}

        public virtual double[] calc_undistorted_coord(int x, int y, double un_x, double un_y)
        {
	    ...
	    static int GetBilinear (double x, double y, int[] colors)
        {
            ...
            for (int i=0 ; i < 3 ; i++)
            {
                ...
                double my = m0 + y * (m1 - m0) ;
          	    crRet[i] = MyImage.SAFECOLOR((int)my) ;
            }
     
            return  MyImage.rgb(crRet[0], crRet[1], crRet[2]);
        }

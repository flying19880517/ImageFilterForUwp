...
{
    public class ZoomBlurFilter : IImageFilter
    {
        int m_length;
        double m_offset_x;
        double m_offset_y;
        ...
        public ZoomBlurFilter(int nLength) : this(nLength, 0.0f, 0.0f)
        {}
		     ...
		     for(int x = 0 ; x < width ; x++){
			      for(int y = 0 ; y < height ; y++){
				      int sr=0, sg=0, sb=0, sa=0;
				       ...
				       imageIn.setPixelColor(x, y, MyImage.SAFECOLOR(sr/sa), MyImage.SAFECOLOR(sg/sa), MyImage.SAFECOLOR(sb/sa));
			      }
		     }
            return imageIn;
        }

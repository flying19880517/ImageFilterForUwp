...
{
    public class ComicFilter : IImageFilter
    {
        ...
        public MyImage process(MyImage input)
        {
            MyImage saturated = saturationFx.process(input.clone());
            MyImage blurred = blurFx.process(saturated);
            input = blender.Blend(saturated, blurred);
            MyImage edge = edgeDetectionFx.process(input.clone());
